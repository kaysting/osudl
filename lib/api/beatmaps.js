...
const getSearchFilterSql = query => {
    ...
    const boolCols = {
        nsfw: 'mapset.is_nsfw',
        explicit: 'mapset.is_nsfw',
        dmca: 'mapset.is_download_disabled',
        copyrighted: 'mapset.is_download_disabled',
        video: 'mapset.has_video',
        hasvideo: 'mapset.has_video'
    };
    const dateCols = {
    ...
    const stringCols = {
        ...
        creator: 'map_search.mapper',
        language: 'map_search.language',
        lang: 'map_search.language',
        genre: 'map_search.genre',
        tags: 'map_search.tags',
        tag: 'map_search.tags',
        source: 'map_search.source'
    };
    const numberCols = {
        stars: 'map.stars',
        sr: 'map.stars',
        ...
        duration: 'map.total_length',
        seconds: 'map.total_length',
        cs: 'map.cs',
        keys: 'map.cs',
        ar: 'map.ar',
        od: 'map.od',
        ...
        health: 'map.hp',
        bpm: 'map.bpm',
        circles: 'map.count_circles',
        notes: 'map.count_circles',
        hits: 'map.count_circles',
        fruits: 'map.count_circles',
        sliders: 'map.count_sliders',
        drumrolls: 'map.count_sliders',
        longnotes: 'map.count_sliders',
        holdnotes: 'map.count_sliders',
        holds: 'map.count_sliders',
        spinners: 'map.count_spinners',
        streams: 'map.count_spinners',
        swells: 'map.count_spinners',
        showers: 'map.count_spinners',
        bananas: 'map.count_spinners'
    };
    const exactCols = {
    ...
    for (const f of activeFilters) {
        ...
        if (boolCols[key]) {
            const col = boolCols[key];
            const truthy = ['1', 't', 'true', 'y', 'yes', 'on'];
            const falsy = ['0', 'f', 'false', 'n', 'no', 'off'];
            const finalVal = truthy.includes(val) ? 1 : falsy.includes(val) ? 0 : null;
        ...
        if (stringCols[key]) {
            ...
            if (op == '=' || op == ':') {
                for (const v of list) {
                    orClauses.push(`map_search MATCH ?`);
                    const sanitizedVal = v.replace(/"/g, '""');
                    sqlParams.push(`${colName}: "${sanitizedVal}"`);
    ...
    // Gemini wrote this optimization
    if (shouldJoinSearchTable) {
        // OPTIMIZATION: If we are doing a text search, the FTS table MUST be the driving table.
        // This stops SQLite from doing a full-table scan to satisfy the ORDER BY clause.
        fromAndJoinClauses = [
            `FROM map_search`,
            `JOIN beatmaps map ON map.id = map_search.beatmap_id`,
        ...
        // Standard join for non-text searches
        fromAndJoinClauses = [
            `FROM beatmaps map`,
            `JOIN beatmapsets mapset ON mapset.id = map.beatmapset_id`,
            shouldJoinPackContents ? `JOIN pack_contents ON pack_contents.mapset_id = mapset.id` : ''
...
const searchBeatmapsSizes = query => {
    ...
    const totals = db
        .prepare(
            ...
            FROM (
                SELECT mapset.id, mapset.novideo_size, mapset.video_size, mapset.has_video
                ${fromAndJoinClauses}
                ${whereClause}
                GROUP BY mapset.id
            )`
...
const searchBeatmaps = (query = '', sort = 'auto', limit = 100, offset = 0) => {
    ...
    const totals = db
        .prepare(
            `SELECT COUNT(*) AS total_beatmapsets FROM (
                SELECT 1
                ${fromAndJoinClauses}
                ${whereClause}
                GROUP BY mapset.id
