...
const searchBeatmaps = (query, sort = 'auto', limit = 100, offset = 0) => {
    ...
    const activeFilters = filters.filter(f => {
        ...
        const col = modeCols[f.key] || statusCols[f.key] || dateCols[f.key] || stringCols[f.key] || numberCols[f.key];
        if (!col) return false;
    ...
    switch (sort) {
        case 'auto':
            textQuery ? (orderBy = `map_search.rank, mapset.time_ranked DESC`) : (orderBy = 'mapset.time_ranked DESC');
            break;
        case 'ranked_desc':
        ...
        case 'bpm_asc':
            ...
            break;
        case 'length_desc':
            orderBy = `map.length DESC`;
            break;
        case 'length_asc':
            orderBy = `map.length ASC`;
            break;
    }
    ...
    const mapsetIds = db
        .prepare(
            ...
            ${textQuery ? `JOIN map_search ON map_search.beatmap_id = map.id` : ''}
