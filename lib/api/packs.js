...
const cachedPackStats = {};
const CACHED_PACK_STATS_TTL = 1000 * 60 * 15;
...
const getPack = packId => {
    ...
    // Check size cache and use if fresh
    const cachedStats = cachedPackStats[packMeta.id];
    const now = Date.now();
    if (cachedStats && cachedStats.expireTime > now) {
        return {
            ...packMeta,
            ...cachedStats.stats
        };
    } else {
        delete cachedPackStats[packMeta.id];
    }
    ...
    if (packMeta.query !== null) {
        ...
        // Update cache
        cachedPackStats[packMeta.id] = {
            expireTime: now + CACHED_PACK_STATS_TTL,
            stats
        };
        return {
            ...packMeta,
 ...
 * @param {string} downloadId The download instance ID
 * @param {number} count The number of downloads to increment by.
 * @returns The updated download instance with `pack`
 */
const incrementPackDownloadInstanceMapCount = (downloadId, count = 1) => {
    db.prepare(
        `UPDATE pack_downloads
        SET count_maps_downloaded = count_maps_downloaded + ?
        WHERE id = ?`
    ).run(count, downloadId);
