 ...
 * @param {boolean} [withSize=true] If false, don't calculate pack size.
 */
const getPack = (packId, withSize = true) => {
    // Fetch the base meta info first
    const packMeta = db.prepare(`SELECT * FROM pack_meta WHERE id = ?`).get(packId);
    if (!packMeta) return null;

    // If size isn't requested, return just pack meta
    if (!withSize) return packMeta;
...
const getPackDownloadInstance = downloadId => {
    ...
    const pack = getPack(downloadInstance.pack_id, false);
    if (!pack) return null;
