...
/**
 * Helper function that fetches and returns formatted beatmapset data from osu. It includes beatmapset and beatmaps properties, each of which have properties that match database column names.
...
const getExistingMapStmt = db.prepare(`SELECT * FROM beatmaps WHERE id = ?`);
const getExistingMapsetStmt = db.prepare(`SELECT * FROM beatmapsets WHERE id = ?`);
...
const saveEntry = db.transaction(entry => {
    ...
    utils.log(`Saved data for mapset ${entry.beatmapset.id}: ${entry.beatmapset.artist} - ${entry.beatmapset.title}`);

    // Wipe existing difficulties and search index rows for this mapset to prevent ghost maps
    db.prepare(`DELETE FROM beatmaps WHERE beatmapset_id = ?`).run(mapset.id);
    db.prepare(`DELETE FROM map_search WHERE beatmapset_id = ?`).run(mapset.id);

    // Insert new list of beatmaps
 ...
 */
const importMapsets = async (mapsetIds, redownloadIfExists = false) => {
    ...
    const logProgress = () => {
        // Don't log if we're just saving a single map
        if (mapsetIds.length == 1) return;
    ...
    for (const id of mapsetIds) {
        try {
            ...
            const existingEntry = getExistingMapsetStmt.get(id);
            ...
            const downloadAndUpload = async () => {
                ...
                if (!entry.beatmapset.is_download_disabled) {
                    // Loop until successful download or unhandled error
                    while (true) {
                        try {
                            utils.log(`Downloading mapset ${id} from osu!...`);
                            // Wait enough time since the last download to avoid hitting rate limits
                            const msSinceLastDownload = Date.now() - lastDownloadTime;
                            await utils.sleep(Math.max(0, env.MS_DELAY_BETWEEN_DOWNLOADS - msSinceLastDownload));
                            // Download mapset
                            const res = await axios({
                                url: env.MAP_DOWNLOAD_TEMPLATE.replace('{mapset_id}', id),
                                method: 'GET',
                                responseType: 'stream',
                                maxRedirects: 5,
                                headers: {
                                    ...
                                    Cookie: env.MAP_DOWNLOAD_TEMPLATE.includes('osu.ppy.sh')
            ...
            // Only download map files if we don't have an entry for this map
            // or if we do and we're forcing a redownload
            if (!existingEntry || (existingEntry && redownloadIfExists)) {
                await downloadAndUpload();
            } else {
                utils.log(`Skipping download of existing mapset ${id}`);
                // Set s3 variables to existing values
                entry.beatmapset.novideo_size = existingEntry.novideo_size;
                entry.beatmapset.novideo_sha256 = existingEntry.novideo_sha256;
                entry.beatmapset.novideo_s3_key = existingEntry.novideo_s3_key;
                entry.beatmapset.video_size = existingEntry.video_size;
                entry.beatmapset.video_sha256 = existingEntry.video_sha256;
                entry.beatmapset.video_s3_key = existingEntry.video_s3_key;
            }
            ...
            if (existingEntry) {
                const oldKeys = [existingEntry.novideo_s3_key, existingEntry.video_s3_key].filter(Boolean);
                const newKeys = [entry.beatmapset.novideo_s3_key, entry.beatmapset.video_s3_key].filter(Boolean);
                for (const oldKey of oldKeys) {
                    if (newKeys.includes(oldKey)) continue;
                    await s3.delete(env.S3_BUCKET, oldKey);
                    utils.log(`Deleted orphaned S3 object ${oldKey}`);
                }
            }
    ...
    try {
        ...
        for (const savedMapset of savedMapsets) {
            try {
                ...
                if (savedMaps.length !== newMaps.length) {
                    isChanged = true;
                ...
                const redownloadMapProps = ['checksum', 'cs', 'ar', 'od', 'hp'];
                ...
                for (const mapId in savedMapsById) {
                    ...
                    if (savedMap && !newMap) {
                        utils.log(
                            `Mapset changed, difficulty ${savedMap.version} was deleted: ${newMapset.artist} - ${newMapset.title}`
                        );
                        isChanged = true;
                        continue;
                    }
                    ...
                    for (const prop of BEATMAP_DB_COLUMNS) {
                        ...
                        isChanged = true;
                        if (
                            redownloadMapProps.includes(prop) &&
                            (prop != 'checksum' || (prop == 'checksum' && savedMap.checksum !== null))
                        ) {
                            needsRedownload = true;
                        }
                    }
                ...
                if (isChanged) {
                    countChanged++;
                    await importMapsets([savedMapset.id], needsRedownload);
                }
                countProcessed++;
...
const getStats = () => {
    const mapsetDetails = db
        .prepare(
            `SELECT
            SUM(COALESCE(b.novideo_size, 0) + COALESCE(b.video_size, 0)) AS count_bytes_stored,
            COUNT(b.id) AS count,
            SUM(
                (COALESCE(dl.count_video, 0) * COALESCE(b.video_size, 0)) + 
                (COALESCE(dl.count_novideo, 0) * COALESCE(b.novideo_size, 0))
            ) AS count_bytes_downloaded
            FROM beatmapsets b
            LEFT JOIN beatmapset_downloads dl ON dl.mapset_id = b.id`
        )
        .get();
    //const mapCount = db.prepare(`SELECT COUNT(*) AS count FROM beatmaps`).get()?.count || 0;
    const stats = [
        {
            name: 'Beatmap files stored',
            value: mapsetDetails.count_bytes_stored,
            value_formatted: utils.formatSize(mapsetDetails.count_bytes_stored)
        },
        {
            name: 'Beatmapsets stored',
            value: mapsetDetails.count,
            value_formatted: utils.formatNumber(mapsetDetails.count)
        },
        {
            name: 'Downloaded by users',
            value: mapsetDetails.count_bytes_downloaded,
            value_formatted: utils.formatSize(mapsetDetails.count_bytes_downloaded)
        }
    ];
    return stats;
};
...
const getMapsetIds = () => {
    return db.prepare(`SELECT id FROM beatmapsets`).all();
};

const getMapIds = () => {
    return db.prepare(`SELECT id FROM beatmaps`).all();
};
