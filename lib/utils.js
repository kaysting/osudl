...
const utils = {
    ...
    /**
     * Log to the Discord webhook if set.
     * @param  {...any} args Arguments to log.
     */
    logToDiscord: (...args) => {
        if (!env.DISCORD_LOGS_WEBHOOK) return;
        return axios.post(env.DISCORD_LOGS_WEBHOOK, {
            content: args
                .map(arg => {
                    switch (typeof arg) {
                        case 'string':
    ...
    /**
     * Format a duration in seconds into a readable m:ss or h:mm:ss string.
     * @param {number} totalSeconds The total length in seconds
    ...
    formatSeconds: totalSeconds => {
        ...
        // Pad seconds with a leading zero so 9 seconds becomes '09'
        const paddedSeconds = s.toString().padStart(2, '0');

        // If it's over an hour, pad the minutes too
        if (h > 0) {
