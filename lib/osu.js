...
// peppy requests that apps make no more than 60 RPM but the API allows for boosting
const MAX_RPM = 60;
const MS_PER_REQUEST = 60000 / MAX_RPM;
...
const makeGetRequest = async (endpoint, params = {}) => {
    ...
    while (true) {
        try {
            ...
            // Thanks Gemini for this async-friendly logic
            let currentWaitTime = 0;
            const now = Date.now();

            // Synchronously claim a spot in the timeline
            if (now < nextRequestTime) {
                currentWaitTime = nextRequestTime - now;
                nextRequestTime += MS_PER_REQUEST; // Add to the queue
            } else {
                nextRequestTime = now + MS_PER_REQUEST; // Reset the queue
            }

            // Now yield the event loop if we need to wait
